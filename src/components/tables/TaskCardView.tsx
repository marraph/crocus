    ...
    return (
        <div className={"grid grid-cols-5 gap-9 pt-4 pb-8 px-8"}>
            {tasks.map((task, index) => (
                <TaskCard key={index} id={task.id} title={task.title} topic={task.topic} team={task.team} project={task.project} status={task.status} createdAt={task.createdAt} createdBy={task.createdBy} />
            ))}
        </div>
    );
}